...
largest_win_index <- which.max(spread)
largest_win_spread <- spread[largest_win_index]

cat("The larges win spread is", largest_win_spread,
    "when the Bears scored", bears[largest_win_index],
    "points and the Opponent scored", opponent[largest_win_index], "points.\n")
```
...
```{r}
extra_games <- matrix(c(37, 9, 17, 17), ncol = 2, byrow = FALSE)
colnames(extra_games) <- c("bears", "opponent")
game_score <- rbind(game_score, extra_games)
print(game_score)
```
...
cat("The area in square inches is", area_result[1], ".\n")
cat("The area in square feet is", area_result[2], ".\n")
```
...
longest_word <- function(words) {
  max_length <- max(nchar(words))
  return(words[nchar(words) == max_length])
}
